...
module FieldValidationHelper
  ...
  def validate_date_of_birth(year, month, day, field)
    ...
    unless invalid_fields != [] || Time.zone.local(year.to_i, month.to_i, day.to_i) < Time.zone.now
      invalid_fields << { field: "#{field}-day", text: t("coronavirus_form.errors.date_order") }
    end
    # Check for date being more than 150 years ago
    unless invalid_fields != [] || Time.zone.local(year.to_i, month.to_i, day.to_i) >= 150.years.ago
  ...
  def validate_postcode(field, postcode)
    if postcode.blank?
      []
    elsif postcode.gsub(" ", "") =~ /^(([A-Z]{1,2}[0-9][A-Z0-9]?|ASCN|STHL|TDCU|BBND|[BFS]IQQ|PCRN|TKCA) ?[0-9][A-Z]{2}|BFPO ?[0-9]{1,4}|(KY[0-9]|MSR|VG|AI)[ -]?[0-9]{4}|[A-Z]{2} ?[0-9]{2}|GE ?CX|GIR ?0A{2}|SAN ?TA1)$/i
      []
    else
